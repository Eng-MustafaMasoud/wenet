      ...
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="mt-1 text-sm text-gray-500">
            {error?.response?.data?.message || 'Failed to load parking state'}
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-lg font-medium text-gray-900">Parking State Report</h2>
          ...
          <div key={zone.id} className="bg-white rounded-lg shadow p-6">
            ...
            <div className="mt-4">
              <div className="flex justify-between text-xs text-gray-500 mb-1">
                <span>Occupancy</span>
                <span>{Math.round((zone.occupied / zone.totalSlots) * 100)}%</span>
              </div>
              <div className="w-full bg-gray-200 rounded-full h-2">
                <div
                  ...
                  style={{
                    width: `${Math.min((zone.occupied / zone.totalSlots) * 100, 100)}%`,
                  }}
                />
              </div>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">
              {report.zones.reduce((sum, zone) => sum + zone.occupied, 0)}
            </div>
            <div className="text-sm text-gray-500">Total Occupied</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">
              {report.zones.reduce((sum, zone) => sum + zone.free, 0)}
            </div>
            <div className="text-sm text-gray-500">Total Free</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-orange-600">
              {report.zones.reduce((sum, zone) => sum + zone.reserved, 0)}
            </div>
            <div className="text-sm text-gray-500">Total Reserved</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">
              {report.zones.reduce((sum, zone) => sum + (zone as any).subscriberCount || 0, 0)}
            </div>
