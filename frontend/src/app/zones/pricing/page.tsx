...
export default function ZonesPricingPage() {
  ...
  const [sortBy, setSortBy] = useState<'name' | 'normal-rate' | 'special-rate'>('name');
  ...
  const pricingData = useMemo(() => {
    if (!zones || !categories) return { zonesByCategory: {}, stats: {} };
    ...
    zones.forEach(zone => {
      ...
      // Calculate stats
      if (zone.specialActive) {
        stats.specialRateZones++;
      } else {
    ...
    const allSpecialRates = zones.map(z => z.rateSpecial);
    stats.avgNormalRate = allNormalRates.reduce((sum, rate) => sum + rate, 0) / allNormalRates.length;
    stats.avgSpecialRate = allSpecialRates.reduce((sum, rate) => sum + rate, 0) / allSpecialRates.length;
  ...
  const filteredZones = useMemo(() => {
    ...
    // Apply filter
    if (filterBy === 'normal') {
      filtered = zones.filter(zone => !zone.specialActive);
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Rate Range</p>
                  <p className="text-2xl font-bold text-gray-900">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  <p className="text-sm font-medium text-gray-600">Special Active</p>
                  <p className="text-2xl font-bold text-gray-900">
          ...
          <div className="bg-white rounded-lg shadow-sm border mb-6">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-medium text-gray-900">Pricing Categories</h3>
            </div>
            <div className="p-6">
                  ...
                  <div key={category.id} className="border border-gray-200 rounded-lg p-4">
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-sm">
                        ...
                        <span className="font-medium">${category.rateNormal}/hr</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">Special Rate:</span>
                        <span className="font-medium text-orange-600">${category.rateSpecial}/hr</span>
                      </div>
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">Zones:</span>
                        <span className="font-medium">
                          {pricingData.zonesByCategory[category.id]?.length || 0}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  {filteredZones.map((zone) => {
                    ...
                    return (
                      <tr key={zone.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-gray-900">{category?.name || zone.categoryId}</div>
                          <div className="text-sm text-gray-500">{category?.description}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className={`text-lg font-bold ${zone.specialActive ? 'text-orange-600' : 'text-gray-900'}`}>
                            ${currentRate}/hr
                          </div>
                          {zone.specialActive && (
