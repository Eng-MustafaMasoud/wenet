...
export default function DashboardPage() {
  ...
  const { isAuthenticated, user, isLoading: authLoading } = useSelector(
  ...
  const isDataLoading = parkingLoading || zonesLoading || categoriesLoading || ticketsLoading || subscriptionsLoading || gatesLoading;
  ...
  const dashboardStats = useMemo(() => {
    ...
    const totalOccupied = parkingState.reduce((sum, zone) => sum + zone.occupied, 0);
    const totalFree = parkingState.reduce((sum, zone) => sum + zone.free, 0);
    const totalReserved = parkingState.reduce((sum, zone) => sum + zone.reserved, 0);
    ...
    const todayTickets = tickets.filter(ticket => 
      ticket.checkoutAt && ticket.checkoutAt.startsWith(today)
    );
    ...
    const avgOccupancyRate = parkingState.length > 0 
      ? parkingState.reduce((sum, zone) => {
          const zoneInfo = zones.find(z => z.id === zone.id);
          return sum + (zoneInfo ? (zone.occupied / zoneInfo.totalSlots) * 100 : 0);
  ...
  const chartData = useMemo(() => {
    ...
    return zones.map(zone => {
      ...
      const occupancyRate = zone.totalSlots > 0 ? (state.occupied / zone.totalSlots) * 100 : 0;
  ...
  const zoneStatusData = useMemo(() => {
    ...
    const available = parkingState.filter(z => z.open && z.availableForVisitors > 0).length;
    const full = parkingState.filter(z => z.open && z.availableForVisitors === 0).length;
    ...
    return tickets
      ...
      .map(ticket => ({
        ...
        duration: ticket.checkoutAt 
          ? Math.round((new Date(ticket.checkoutAt).getTime() - new Date(ticket.checkinAt).getTime()) / (1000 * 60 * 60) * 10) / 10
          : null,
      }));
  }, [tickets]);
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="max-w-7xl mx-auto">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{dashboardStats.totalSlots}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{dashboardStats.totalOccupied}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{dashboardStats.totalFree}</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <div className="flex items-center">
                ...
                <div className="ml-4">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{dashboardStats.occupancyRate.toFixed(1)}%</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Zone Occupancy</h3>
              <div className="h-80">
            ...
            {/* Zone Status Distribution */}
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Zone Status Distribution</h3>
              <div className="h-80">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
            ...
            <div className="bg-white p-6 rounded-lg shadow-sm border">
              ...
              <div className="space-y-4">
                <div className="flex justify-between">
                  ...
                  <span className="font-semibold">{dashboardStats.totalGates}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Total Zones</span>
                  <span className="font-semibold">{dashboardStats.totalZones}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Categories</span>
                  <span className="font-semibold">{dashboardStats.totalCategories}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Active Subscriptions</span>
                  <span className="font-semibold text-green-600">{dashboardStats.activeSubscriptions}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Active Tickets</span>
                  <span className="font-semibold text-blue-600">{dashboardStats.activeTickets}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-600">Revenue Today</span>
                  <span className="font-semibold text-green-600">${dashboardStats.revenueToday.toFixed(2)}</span>
                </div>
              </div>
            </div>
                    ...
                    <div key={activity.id} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-b-0">
                      <div>
                        <div className="text-sm font-medium text-gray-900">
                          {activity.type === 'subscriber' ? 'Subscriber' : 'Visitor'} Check-in
                        </div>
                        <div className="text-xs text-gray-500">{activity.zoneId}</div>
                      </div>
                      <div className="text-right">
                        <div className="text-xs text-gray-500">{activity.checkinAt}</div>
                        {activity.duration && (
                          <div className="text-xs text-gray-400">{activity.duration}h</div>
                        )}
                      </div>
                    </div>
                  ))
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <h3 className="text-lg font-medium text-gray-900 mb-4">Zone Performance Overview</h3>
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={chartData}>
                  ...
                  <Tooltip />
                  <Bar yAxisId="left" dataKey="occupied" fill="#EF4444" name="Occupied" />
                  <Bar yAxisId="left" dataKey="free" fill="#10B981" name="Available" />
                  <Line yAxisId="right" type="monotone" dataKey="occupancyRate" stroke="#3B82F6" strokeWidth={2} name="Occupancy %" />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
          </div>
